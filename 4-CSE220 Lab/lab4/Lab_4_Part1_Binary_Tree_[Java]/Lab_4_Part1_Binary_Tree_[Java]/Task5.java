...
public class Task5 {
    ...
    public static Integer maxTree( BTNode root ){
        if (root == null) {
            return null; // Return null if the tree or subtree is empty
        }
        ...
        // Compare current node's value with the max of the left subtree
        if (leftMax != null && leftMax > max) {
            max = leftMax;
        }
        
        // Compare the current running max with the max of the right subtree
